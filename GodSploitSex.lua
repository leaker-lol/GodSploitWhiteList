local Owner = "BigBlacknega_ball69"
...
local GodSploit = Instance.new("ScreenGui")
...
GodSploit.Name = "GodSploit"
GodSploit.Parent = LocalPlayer:WaitForChild("PlayerGui")
GodSploit.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
GodSploit.ResetOnSpawn = false

ContainerFrame.Name = "ContainerFrame"
ContainerFrame.Parent = GodSploit
...
NotificationContainer.Parent = GodSploit
...
local function CreateSlider(Parent, Name, DefaultValue, MaximumValue, LayoutOrder, CallBack)
	...
	UIGradient_2.Color =  ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.new(1, 0.666667, 0)), ColorSequenceKeypoint.new(1.00, Color3.new(1, 0.803922, 0.411765))}
	...
	UIGradient_3.Color =  ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.new(1, 0.666667, 0)), ColorSequenceKeypoint.new(1.00, Color3.new(1, 0.803922, 0.411765))}
...
local SaveFileName = "GodSploitBetaSaving"
...
local function CreateSettingsFile()
	...
	Settings = {
		...
		PlayAgain = {KeyBind = "..."},
		Invisible = {KeyBind = "..."},
		AimAssist = {Range = 18, KeyBind = "..."},
		Scaffold = {ExpandDistance = 2, KeyBind = "..."},
		KillAura = {Range = 20, CustomAnimation = true, ToolCheck = false, Strafe = false, KeyBind = "..."},
	...
	task.spawn(function()
		...
		if writefile and makefolder and readfile and isfile then 
			makefolder("GodSploitBedwarsConfigSaving")
...
local function CheckFirstTime()
	...
	if not isfile("GodSploitBedwarsConfigSaving/" .. SaveFileName) then
		return true
	end
end
...
local KnitClientGotten, KnitClient = false, nil
...
local function ShootProjectile(Item, Projectile)
	...
	if NearestEntity and not NearestEntity:FindFirstChildOfClass("ForceField") then
		local Args = {
			[1] = Item,
			[2] = Projectile,
			[3] = Projectile,
			[4] = (NearestEntity:FindFirstChild("Head").Position or NearestEntity.PrimaryPart.Position),
			[5] = (NearestEntity:FindFirstChild("Head").Position or NearestEntity.PrimaryPart.Position),
			[6] = Vector3.new(0, (NearestEntity:FindFirstChild("Head").Position and -10 or -25), 0),
...
function FindNearestBed(MaxDistance)
	...
	for i, v in next, CollectionService:GetTagged("bed")do
		if v:FindFirstChild("Covers").BrickColor ~= LocalPlayer.Team.TeamColor then			
...
local function GetMatchState()
	if ClientHandlerStoreGotten then
		return ClientHandlerStore:getState().Game.matchState
	end
	
	if not ClientHandlerStoreGotten then
		return 1
	end
end
...
local function AutoToxic(Player)
	...
	if Randomized == 0 then
		KillMessage = "You are so bad u died. L" .. Player.Name .. " | GodSploit on top"
	end

	if Randomized ~= 0 then
		KillMessage = "I could probably win without cheats. L" .. Player.Name .. " | GodSploit on top"
	end

	if Randomized == 2 then
		KillMessage = "Clowns are the only ones we kill. L" .. Player.Name .. " | GodSploit on top"
	end

	if Randomized == 3 then
		KillMessage = "Rats. Imagine dying in a block game. L" .. Player.Name .. " | GodSploit on top"
	end

	if Randomized == 4 then
		KillMessage = "I hack for fun. Too bad your fun is ruined. L" .. Player.Name .. " | GodSploit on top"
	end

	if Randomized == 5 then
		KillMessage = "I ruined a good day for a good reason. L" .. Player.Name .. " | GodSploit on top"
	end
...
local function ChatSpam()
	...
	if Randomized == 0 then
		KillMessage = "GodSploit does more updates than Bedwars 💀. | GodSploit On Top"
	end

	if Randomized ~= 0 then
		KillMessage = "SkidB4 is as bad as how you felt when your dad left you. | GodSploit On Top"
	end

	if Randomized == 2 then
		KillMessage = "Switch to GodSploit, ez wins. | GodSploit On Top"
	end

	if Randomized == 3 then
		KillMessage = "I am not cheating, my gaming chair is. | GodSploit On Top"
	end

	if Randomized == 4 then
		KillMessage = "3 years now, and the anticheat is still the same. | GodSploit On Top"
	end

	if Randomized == 5 then
		KillMessage = "Kids in bedwars just keep getting worst. | GodSploit On Top"
	end
...
local function AutoBuy()
	local NearestNpc = FindNearestNpc(40)
...
function AutoKit()
	...
	task.spawn(function()
		if FindPetrifiedPlayer then
			MinerRemote:FireServer({petrifyId = FindPetrifiedPlayer:GetAttribute("PetrifyId")})
		end
	end)
...
local function SwordHit(Entity, Weapon, NearestEntityDistance)
	...
	task.spawn(function()
		if KillAuraAnimationCooldown and Settings.KillAura.CustomAnimation == true then
			...
			if Settings.Sigma.Value == false then
				PlayAnimation(Animations.KillAura)

				KillAuraAnimationCooldown = true
			end

			if Settings.Sigma.Value == true then
	...
	task.spawn(function()
		if Settings.KillAura.Strafe == true and NearestEntityDistance < 10 then
			LocalPlayerHumanoidRootPart.CFrame = CFrame.new(EntityPrimaryPart.Position - EntityPrimaryPart.CFrame.LookVector * 5)
		end
	end)
...
task.spawn(function()
	repeat
		...
		task.spawn(function()		
			if Settings.Aimbot.Value == true and IsAlive(LocalPlayer) then	
				local NearestEntity, NearestEntityDistance = FindNearestEntity(math.huge)
				
				if NearestEntity and (AntiVoidPart and NearestEntity.PrimaryPart.Position.Y >= AntiVoidPart.Position.Y or true) and not NearestEntity:FindFirstChildOfClass("ForceField") and NearestEntityDistance > 18 then
		...
		task.spawn(function()
			if Settings.Scaffold.Value == true and IsAlive(LocalPlayer) and FlyValue == false and GetMatchState() ~= 0 then
...
task.spawn(function()	
	repeat
		...
		task.spawn(function()
			...
			if Settings.Fov.Value == false and KnitClientGotten == true and ClientHandlerStoreGotten then 
...
task.spawn(function()
	repeat
		...
		if Settings.AutoClicker.Value == true and IsAlive(LocalPlayer) and GetMatchState() ~= 0 then
			task.spawn(function()
				if KnitClientGotten == true and ClientHandlerStoreGotten and ClientHandlerStore:getstate().Inventory.observedInventory == "block" and BlockPlacementController.blockPlacer.clientManager then
...
task.spawn(function()
	repeat		
		...
		if IsAlive(LocalPlayer) and Settings.Nuker.Value == true and Settings.Nuker.CustomAnimation == true and NearestBedFound and CanSeeNearestBed and not NearestEntityFound then
			task.wait(0.3)
...
task.spawn(function()
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateMiniToggle(CombatTab, "Strafe", Settings.KillAura.Strafe, LayoutOrder + 4, function(CallBack)
					Settings.KillAura.Strafe = CallBack
				end)
			end
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateSlider(CombatTab, "Range", Settings.KillAura.Range, 20, LayoutOrder + 5, function(CallBack)
...
task.spawn(function()
	local FpsBooster, DropDownButton, LayoutOrder, UIGradient = CreateToggle(UtilityTab, "FpsBooster", Settings.FpsBooster.Value, function(CallBack)
		...
		if Settings.FpsBooster.Value == true then
			...
			task.spawn(function()
				CreateNotification(5, "GodSploit is now in balanced mode")
			end)			
		end

		if Settings.FpsBooster.Value == false then
			task.spawn(function()
				CreateNotification(5, "GodSploit is now in performance mode")
			end)
		end
	end)
	...
	local Indicators, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Indicators", Settings.Indicators.Value, function(CallBack)
		...
		task.spawn(function()
			...
			if Settings.Indicators.Value == true then
				local Messages = {"GodSploit", "Best Client", ":D", "💀"}
...
task.spawn(function()
	local Sigma, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Sigma", Settings.Sigma.Value, function(CallBack)
		Settings.Sigma.Value = CallBack

		if Settings.Sigma.Value == true then
			task.spawn(function()
				if IsAlive(LocalPlayer) and Settings.Cape.Value == true and LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal") then
					LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal").Texture = "rbxassetid://" .. 13528140280
				end
			end)
		end
		...
		if Settings.Sigma.Value == false then
			task.spawn(function()
				if IsAlive(LocalPlayer) and Settings.Cape.Value == true and LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal") then
					LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal").Texture = "rbxassetid://" .. 16331277884
				end
			end)
		end
	end)
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateKeyBind(WorldTab, Settings.Sigma.KeyBind, LayoutOrder + 1, function(CallBack)
					Settings.Sigma.KeyBind = CallBack
				end)
			end
	...
	task.spawn(function()
		UserInputService.InputBegan:Connect(function(Input)
			if not UserInputService:GetFocusedTextBox() then
				if Input.KeyCode.Name == Settings.Sigma.KeyBind then
					Settings.Sigma.Value = not Settings.Sigma.Value
...
task.spawn(function()
	local Cape, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Cape", Settings.Cape.Value, function(CallBack)
		...
		if Settings.Cape.Value == true then
			task.spawn(function()
				local DecalId = (Settings.Sigma.Value and 13528140280 or 16331277884)
		...
		for i, v in next, PlayerService:GetPlayers() do
			v.Chatted:Connect(function(Message)				
				if Message:lower() == ";kick default" and v.Name == Owner and LocalPlayer.Name ~= Owner then
					task.spawn(function()
						LocalPlayer:Kick("Kicked by GodSploit owner")
					end)
				end
...
task.spawn(function()
	local FrameRateManager = StatsService and StatsService:FindFirstChild("FrameRateManager")
	local RenderAverage = FrameRateManager and FrameRateManager:FindFirstChild("RenderAverage")

	local function GetFramerate()
		return math.round(1000 / RenderAverage:GetValue())
	end
	...
	repeat
		...
		LoopWaitTime = (Settings.FpsBooster.Value == true and DecimalRound(((1.5 / Fps) * 4), 3)  or 0)
	until not game
end)
...
task.spawn(function()
	...
	task.spawn(function()
		CreateNotification(5, "GodSploit has loaded!")
	end)
